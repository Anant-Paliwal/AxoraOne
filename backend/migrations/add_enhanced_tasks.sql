...
-- Adds support for subtasks, events, birthdays, blocked status, and better calendar sync

-- Add new columns to tasks table
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS parent_task_id UUID REFERENCES tasks(id) ON DELETE CASCADE;
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS event_type TEXT DEFAULT 'task' CHECK (event_type IN ('task', 'event', 'birthday', 'reminder', 'milestone'));
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS created_from TEXT DEFAULT 'manual' CHECK (created_from IN ('page', 'skill', 'ask', 'manual', 'calendar'));
...
ALTER TABLE tasks ADD CONSTRAINT tasks_status_check CHECK (status IN ('todo', 'in-progress', 'done', 'blocked'));
...
-- Function to get subtasks count
