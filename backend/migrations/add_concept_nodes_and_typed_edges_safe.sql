-- Safe Migration: Add Concept Nodes and Enhanced Typed Edges
...
-- 2. Add indexes for concepts (if not exists)
CREATE INDEX IF NOT EXISTS idx_concepts_user_workspace ON public.concepts(user_id, workspace_id);
CREATE INDEX IF NOT EXISTS idx_concepts_name ON public.concepts(name);
CREATE INDEX IF NOT EXISTS idx_concepts_importance ON public.concepts(importance_score DESC);
CREATE INDEX IF NOT EXISTS idx_concepts_usage ON public.concepts(usage_count DESC);
...
-- 4. Update edge types constraint (drop and recreate)
...
-- 5. Add edge metadata columns (if not exists)
DO $$ 
BEGIN
...
-- 6. Create indexes for new edge features (if not exists)
...
-- 7. Create or replace concept extraction function
...
-- 8. Create trigger for concept extraction (drop if exists)
...
-- 9. Create or replace graph insights function
...
-- 10. Create or replace learning path function
...
BEGIN
    WITH RECURSIVE path_search AS (
        ...
        AND ps.depth < 5
        AND NOT (e.source_id = ANY(ps.path_ids))
...
DO $$
BEGIN
    RAISE NOTICE 'Migration completed successfully! Concept nodes and enhanced edges are ready.';
END $$;
