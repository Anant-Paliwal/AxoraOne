-- ============================================
-- SKILL PROGRESS DIAGNOSTIC SCRIPT
...
SELECT 
    ...
    -- Calculate progress (Beginner level requirements)
    ROUND(
        CAST(
            (
...
SELECT 
    '=== QUICK FIX TEST ===' as section;

-- This will insert a test contribution if none exist
-- Uncomment to run:
...
WHERE s.workspace_id IS NOT NULL
AND NOT EXISTS (
    SELECT 1 FROM skill_contributions WHERE skill_id = s.id
)
LIMIT 1
RETURNING 
    '✅ Test contribution created! Refresh skill page to see 20% progress' as result;
*/
...
SELECT 
    ...
    COUNT(DISTINCT skill_id) as value
FROM skill_evidence
UNION ALL
SELECT 
...
SELECT 
    ...
    ROUND(CAST(COALESCE(SUM(impact_score), 0) AS numeric), 2) as value
FROM skill_contributions;
...
For testing, uncomment the INSERT statement above to manually
create a test contribution and verify the progress calculation works.
*/
